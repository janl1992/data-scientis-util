...
def dump_model(grid_search, name='best_estimator', path='../models'):
    now = datetime.now()
    date_time = now.strftime("%m_%d_%Y-%H%M%S")

    with open("%s/score_%s.txt"%(path, date_time), "w") as text_file:
        text_file.write("Validation score: %s" % grid_search.best_score_)

    model_name=grid_search.best_score_.replace('.',',')
    joblib.dump(grid_search.best_estimator_, "%s/%s-%s.pkl" % (path, model_name, date_time))
...
def load_model(name, path='../models'):
    ...
    raise FileNotFoundErrorimport pandas as pd
from datetime import datetime
import csv
import joblib
from datetime import datetime
